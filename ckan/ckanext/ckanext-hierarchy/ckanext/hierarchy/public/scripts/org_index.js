...
$(function() {
  function buildIndex(parentDiv) {
    var out = []
    parentDiv.find('> .organization').each(function(i,row) {
      row = $(row);
      var link = row.find('> .org-row > a:first');
      out.push({
        div        : row,
        link       : link,
        rawtext    : link.text(),
  ...
  var container = $('.org-hierarchy');
  var searchBox = $('input#js-search');
  var resultCountBox = $('.org-search-result');
...
//  assert (searchBox.length);
  ...
  function onChange(e) {
    ...
    window.clearTimeout(window.edc_org_timeout);
    window.edc_org_timeout = window.setTimeout(function() {
